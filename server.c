...
void avant_partie(Player* info_joueur){
    ...
    while(1)
    {
        ...
        else if(strcmp(message, "CHSIZ") == 0) // [CHSIZ_m_h_w***]
        {
            chsize(info_joueur);
        }
        else
...
void newpl_regis(Player* info_joueur, uint8_t is_regis)
{
    ...
    {
        // Envoi du message [REGNO***]
        printf("Le port UDP est déjà utilisé.\n");
        regno(info_joueur);
        return;
    }
...
void welco(Player* info_joueur)
{
    ...
    if(info_joueur == get_first_player(info_joueur)) 
    {
        ...
        char commande[30], labyrinthe[30];
        sprintf(commande, "java labyrinthe %u %u %u", (parties[info_joueur -> m].h/2), parties[info_joueur -> m].l/2, info_joueur -> m);
        sprintf(labyrinthe, "labyrinthe%u.txt", info_joueur -> m);
        system(commande);

        // Attribution des valeurs de la partie qui commence
        parties[info_joueur -> m].plateau = parse_txt(labyrinthe);
...
void partie_en_cours(Player* info_joueur)
{
    ...
    while(1)
    {
        ...
        else if(strcmp(message, "IQUIT") == 0) // [IQUIT***]
        {
            read(info_joueur -> sock_tcp, buffer, 3); // ***
            gobye(info_joueur);
            if(parties[info_joueur -> m].s - 1 == 0)
            {
                printf("La partie est terminée, il n'y a plus de joueurs dedans.\n");
            }
            else
            {
...
void move(char d, Player* info_joueur)
{
    ...
    switch (d) {
        case 'U':
            for(uint16_t i = y ; i >= y - depl ; i--) {
                tmp += ghost_captured(info_joueur, x, i);
        ...
        case 'L':           
            for(uint16_t i = x ; i >= x - depl ; i--) {
                tmp += ghost_captured(info_joueur, i, y);
    ...
    if(tmp > 0){
        ...
        printf("Il reste %d fantomes\n", parties[info_joueur -> m].f);
        printf("tmp: %d\n", tmp);
    }
...
void deplacer_fantomes_aleatoirement(Player* info_joueur)
{
    ...
    if(info_joueur == get_first_player(info_joueur)) 
    {
        for(uint8_t z = 0; z < parties[info_joueur -> m].f; z++)
        {
                do
                {
                    x = rand() % parties[info_joueur -> m].l;
                    y = rand() % parties[info_joueur -> m].h;
                } while(parties[info_joueur -> m].plateau[y][x] != ESPACE_VIDE);

                parties[info_joueur -> m].fantomes[z][0] = x;
                parties[info_joueur -> m].fantomes[z][1] = y;
...
// Renvoie 1 si tous les joueurs du lobby sont prêt à commencer une partie
...
// Retourne le pointeur du premier joueur de la partie
...
// Retourne le pointeur du joueur avec le plus haut score de la partie
...
// Déconnecte le joueur du serveur et supprime ses données
void deconnexion(Player* info_joueur) 
{
    if(parties[info_joueur -> m].etat == 1 || parties[info_joueur -> m].etat == 2)
    {
        parties[info_joueur -> m].s--;
    }
    parties[info_joueur -> m].joueurs[info_joueur -> i] = NULL;
...
// Transforme un nombre en chaîne de caractères de taille 3
...
// Transforme un nombre en chaîne de caractères de taille 4
...
int ghost_captured(Player* info_joueur, uint16_t x, uint16_t y)
{
    uint16_t score = 0;
    int tmp = 0;
    char caught[31], stockage_x[4], stockage_y[4];
    if(parties[info_joueur -> m].plateau[y][x]==1 || x < 0 || y < 0 || x > parties[info_joueur -> m].l || y > parties[info_joueur -> m].h){
        return 0;
    }
 
    for(uint8_t i = 0; i < parties[info_joueur -> m].f; i++)
    {
        if(x == parties[info_joueur -> m].fantomes[i][0] && y == parties[info_joueur -> m].fantomes[i][1])
        {
            if(parties[info_joueur -> m].fantomes[i][0] > -1 && parties[info_joueur -> m].fantomes[i][1] > -1)
            {
                tmp++;
                parties[info_joueur -> m].fantomes[i][0] = -1;
                parties[info_joueur -> m].fantomes[i][1] = -1;
                tostrtmp(stockage_x, x);
                tostrtmp(stockage_y, y);
                
                score += atoi(info_joueur -> p) + 10;
                tostrtmp(info_joueur -> p, score);
                sprintf(caught, "SCORE %s %s %s %s+++", info_joueur->id, info_joueur->p, stockage_x, stockage_y);
                send_multicast(info_joueur, caught, strlen(caught));
            }
        }
    }
    return tmp;
